-- MySQL Script generated by MySQL Workbench
-- Sun Oct 23 20:00:56 2022
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';

-- -----------------------------------------------------
-- Schema mydb
-- -----------------------------------------------------
-- -----------------------------------------------------
-- Schema db
-- -----------------------------------------------------

-- -----------------------------------------------------
-- Schema db
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `db` DEFAULT CHARACTER SET utf8 ;
USE `db` ;

-- -----------------------------------------------------
-- Table `db`.`alembic_version`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db`.`alembic_version` (
  `version_num` VARCHAR(32) NOT NULL,
  PRIMARY KEY (`version_num`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb3;


-- -----------------------------------------------------
-- Table `db`.`roles`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db`.`roles` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `login` VARCHAR(255) NOT NULL,
  `default` TINYINT(1) NULL DEFAULT NULL,
  `permissions` INT NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `login_UNIQUE` (`login` ASC) VISIBLE)
ENGINE = InnoDB
AUTO_INCREMENT = 5
DEFAULT CHARACTER SET = utf8mb3;


-- -----------------------------------------------------
-- Table `db`.`users`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db`.`users` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `login` VARCHAR(45) NOT NULL,
  `email` VARCHAR(45) NOT NULL,
  `is_confirmed` TINYINT(1) NULL DEFAULT NULL,
  `hash` VARCHAR(255) NOT NULL,
  `role_id` INT NULL DEFAULT NULL,
  `token` VARCHAR(1000) NULL DEFAULT NULL,
  `online` DATETIME NULL DEFAULT NULL,
  `created` DATETIME NOT NULL,
  `path` VARCHAR(255) NULL DEFAULT NULL,
  `name` VARCHAR(64) NULL DEFAULT NULL,
  `location` VARCHAR(255) NULL DEFAULT NULL,
  `about_me` TEXT NULL DEFAULT NULL,
  `passlen` INT NULL DEFAULT NULL,
  `thumb_path` VARCHAR(255) NULL DEFAULT NULL,
  `dislike_id` INT NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `email_UNIQUE` (`email` ASC) VISIBLE,
  UNIQUE INDEX `login_UNIQUE` (`login` ASC) VISIBLE,
  INDEX `Role_idx` (`role_id` ASC) VISIBLE,
  INDEX `fk_users_articles1_idx` (`dislike_id` ASC) VISIBLE,
  CONSTRAINT `Role`
    FOREIGN KEY (`role_id`)
    REFERENCES `db`.`roles` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
AUTO_INCREMENT = 31
DEFAULT CHARACTER SET = utf8mb3;


-- -----------------------------------------------------
-- Table `db`.`articles`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db`.`articles` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `author_id` INT NULL DEFAULT NULL,
  `text` TEXT NULL DEFAULT NULL,
  `path` VARCHAR(255) NULL DEFAULT NULL,
  `created_at` DATETIME NULL DEFAULT NULL,
  `description` TEXT NULL DEFAULT NULL,
  `article_name` VARCHAR(255) NULL DEFAULT NULL,
  `heading` VARCHAR(255) NOT NULL,
  `cover_path` VARCHAR(255) NULL DEFAULT NULL,
  `attitude` INT NULL DEFAULT NULL,
  `article_position` INT NULL,
  `confirmed` TINYINT(1) NULL,
  `tags` VARCHAR(255) NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `article_name_UNIQUE` (`article_name` ASC) VISIBLE,
  INDEX `Autor_idx` (`author_id` ASC) VISIBLE,
  UNIQUE INDEX `cover_path_UNIQUE` (`cover_path` ASC) VISIBLE,
  UNIQUE INDEX `path_UNIQUE` (`path` ASC) VISIBLE,
  UNIQUE INDEX `article_position_UNIQUE` (`article_position` ASC) VISIBLE,
  CONSTRAINT `Author_id`
    FOREIGN KEY (`author_id`)
    REFERENCES `db`.`users` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
AUTO_INCREMENT = 147
DEFAULT CHARACTER SET = utf8mb3;


-- -----------------------------------------------------
-- Table `db`.`user_art_attitude`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db`.`user_art_attitude` (
  `articles_id` INT NOT NULL,
  `users_id` INT NOT NULL,
  `attitude` TINYINT(1) NULL,
  PRIMARY KEY (`articles_id`, `users_id`),
  INDEX `fk_userarticle_users1_idx` (`users_id` ASC) VISIBLE,
  CONSTRAINT `fk_userarticle_articles1`
    FOREIGN KEY (`articles_id`)
    REFERENCES `db`.`articles` (`id`),
  CONSTRAINT `fk_userarticle_users1`
    FOREIGN KEY (`users_id`)
    REFERENCES `db`.`users` (`id`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb3;


-- -----------------------------------------------------
-- Table `db`.`comments`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db`.`comments` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `text` TEXT NULL,
  `created_at` DATETIME NULL,
  `disabled` TINYINT(1) NULL,
  `path` VARCHAR(255) NULL,
  `users_id` INT NOT NULL,
  `articles_id` INT NOT NULL,
  PRIMARY KEY (`id`, `users_id`, `articles_id`),
  INDEX `fk_comments_users1_idx` (`users_id` ASC) VISIBLE,
  INDEX `fk_comments_articles1_idx` (`articles_id` ASC) VISIBLE,
  CONSTRAINT `User`
    FOREIGN KEY (`users_id`)
    REFERENCES `db`.`users` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `Article`
    FOREIGN KEY (`articles_id`)
    REFERENCES `db`.`articles` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
